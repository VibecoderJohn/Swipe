...
class User:
    ...
    def create(cls, email, phone, password):
        ...
        return cls.collection.insert_one(user_data).inserted_id

    @classmethod
    def find_by_email_or_phone(cls, identifier):
        return cls.collection.find_one({"$or": [{"email": identifier}, {"phone": identifier}]})
...
class Biometric:
    ...
    def enroll(cls, user_id, biometric_type, template):
        ...
        if cls.collection.find_one({"userId": ObjectId(user_id), "type": biometric_type}):
        ...
        return cls.collection.insert_one(biometric_data).inserted_id

    @classmethod
    def list_for_user(cls, user_id):
        biometrics = cls.collection.find({"userId": ObjectId(user_id)})
    ...
    def delete(cls, biometric_id, user_id):
        result = cls.collection.delete_one({"_id": ObjectId(biometric_id), "userId": ObjectId(user_id)})
        return result.deleted_count > 0

    @classmethod
    def get_template(cls, user_id, biometric_type):
        biometric = cls.collection.find_one({"userId": ObjectId(user_id), "type": biometric_type})
        if biometric:
...
class Transaction:
    ...
    def initiate(cls, user_id, amount, recipient, account_id):
        ...
        return cls.collection.insert_one(transaction_data).inserted_id

    @classmethod
    def authenticate(cls, transaction_id, user_id, biometric_types, templates):
        transaction = cls.collection.find_one({"_id": ObjectId(transaction_id), "userId": ObjectId(user_id)})
        if not transaction or transaction["status"] != "initiated":
            raise ValueError("Invalid transaction")
        if len(biometric_types) < 2 and transaction["amount"] > 10000:  # > N100, assume kobo
        ...
        for b_type, template in zip(biometric_types, templates):
            ...
            if stored_template != template:  # Simulate matching
